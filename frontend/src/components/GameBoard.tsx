        ...
        <div className="battlefield">
          <div className="opponent-field">
            {gameState.battlefield[opponentKey].map((card: CardType) => {
              const updatedCard = {
                ...card,
                imageUrl: `/monsters/${card.id}.png`, // Ensure the path is correct
              };
              console.log('Opponent card image URL:', updatedCard.imageUrl);
              return (
                <Card
                  key={card.id}
                  card={updatedCard}
                  ...
                  onError={(e: React.SyntheticEvent<HTMLImageElement, Event>) => {
                    console.error(`Failed to load image for card ${card.id}: ${updatedCard.imageUrl}`);
                    e.currentTarget.src = cardBackMonster; // Fallback image
                  }}
                />
              );
            })}
          ...
          <div ref={dropRef as unknown as React.RefObject<HTMLDivElement>} className={`player-field ${isOver ? 'field-highlight' : ''}`}>
            {gameState.battlefield[playerKey].map((card: CardType) => {
              ...
              console.log('Player card image URL:', updatedCard.imageUrl);
              return (
                <Card
                  key={card.id}
                  card={updatedCard}
          ...
          <div className="player-hand">
            {gameState.players[playerKey].hand.length > 0 ? (
              gameState.players[playerKey].hand.map((card: CardType) => {
                ...
                console.log('Player hand card image URL:', updatedCard.imageUrl);
                return (
                  <Card
                    key={card.id}
                    card={updatedCard}
